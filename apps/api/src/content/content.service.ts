...
export class ContentService {
  ...
  async generate(input: {
    ...
    const strategySeed = {
      ...
      ...(mediaType === 'VIDEO' && resolvedVideoProvider
  ...
  async processQueuedGeneration(contentId: string, businessId: string) {
    ...
    if (!content) return;
    try {
      const stored = content.strategy as Record<string, unknown> | null;
      const storedVideoProvider =
  ...
  private async generateVideoAsset(input: {
    ...
    const video = await this.videos.generate({
      ...
      provider: (input.provider as any) ?? undefined,
  ...
  async upsertSocialConfig(input: {
    ...
    const videoProviderRaw = input.preferredVideoProvider?.trim().toLowerCase() || null;
    const videoProvider =
      videoProviderRaw && ['kie', 'fal', 'veo'].includes(videoProviderRaw) ? videoProviderRaw : videoProviderRaw ? (() => { throw new BadRequestException('Provider de video inválido: kie, fal o veo'); })() as null : null;
    const videoModel = input.preferredVideoModel?.trim() || null;
    ...
    const saved = await this.prisma.socialContentConfig.upsert({
      ...
      update: {
        ...
        ...(input.preferredVideoProvider !== undefined
          ? { preferredVideoProvider: videoProvider }
          : {}),
        ...(input.preferredVideoModel !== undefined
          ? { preferredVideoModel: videoModel }
          : {}),
      },
    });
  ...
  private async resolveVideoConfig(
    businessId: string,
    overrideProvider?: string | null,
    overrideModel?: string | null,
  ): Promise<{ provider: string | null; model: string | null }> {
    const providerOverride = overrideProvider?.trim() || null;
    const modelOverride = overrideModel?.trim() || null;
    ...
    const config = await this.prisma.socialContentConfig.findUnique({
      where: { businessId },
      select: { preferredVideoProvider: true, preferredVideoModel: true },
    });
    const provider = config?.preferredVideoProvider?.trim() || null;
    const model = config?.preferredVideoModel?.trim() || null;
    if (provider || model) {
      return { provider, model };
    }

    // Fallback a env — VideoRoutingService ya lo hace, pero devolvemos null para que use env
